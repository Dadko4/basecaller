...
eval_ds = read_raw_data_sets(r"eval/", max_segments_num=10000)
...
#     noise = GaussianNoise(0.01)(masking)
    ...
    pad = ZeroPadding1D(padding=(0, 310))(dropout1)
    ...
    cnn0 = Conv1D(filters=units, kernel_size=5, strides=1, activation='relu',
               ...
               name='conv_1')(pad)
    dropout2 = TimeDistributed(Dropout(dropout), name='dropout_2')(cnn0)

    cnn1 = Conv1D(filters=units, kernel_size=5, strides=1, activation='relu',
               kernel_initializer=lstm_init, bias_initializer=initializer,
               name='conv_2')(dropout2)
    dropout3 = TimeDistributed(Dropout(dropout), name='dropout_3')(cnn1)

    cnn2 = Conv1D(filters=units, kernel_size=5, strides=2, activation='relu',
               kernel_initializer=lstm_init, bias_initializer=initializer,
               name='conv_3')(dropout3)
    dropout4 = TimeDistributed(Dropout(dropout), name='dropout_4')(cnn2)

    dense2 = TimeDistributed(Dense(units=units, kernel_initializer=initializer,
                                   bias_initializer=initializer, activation='relu'), 
                            name='fc_2')(dropout4)
    dropout5 = TimeDistributed(Dropout(dropout), name='dropout_5')(dense2)
    dense3 = TimeDistributed(Dense(units=units, kernel_initializer=initializer,
                                   bias_initializer=initializer, activation='relu'), 
                            name='fc_3')(dropout5)
    dropout6 = TimeDistributed(Dropout(dropout), name='dropout_6')(dense3)
    blstm0 = Bidirectional(LSTM(units, return_sequences=True, kernel_initializer=lstm_init, 
                                dropout=dropout))(dropout6)
    ...
    dense4 = TimeDistributed(Dense(units=units, kernel_initializer=initializer,
                                   bias_initializer=initializer,activation='relu'),
                                   name='fc_4')(blstm2)
    dropout7 = TimeDistributed(Dropout(dropout), name='dropout_7')(dense4)

    dense5 = TimeDistributed(Dense(labels + 1, name="fc_5"))(dropout7)
    y_pred = Activation('softmax', name='softmax')(dense5)
    network = CTCModel([input_data], [y_pred])
    network.load_model(path_dir='CNN_2_model', optimizer=Nadam(learning_rate=0.0005),
                       file_weights='CNN_2_model/CNN_weights.h5')
    return network
...
K.clear_session()
network = create_CNN_network(features, labels, padding_value, units=256)
print(network.summary())
X_val, y_val = prepare_data(10000, train=False).__next__()

mc = ModelCheckpoint('best_CNN_2_model.h5', monitor='val_loss', verbose=1,
...
history = network.fit_generator(prepare_data(batch_size), steps_per_epoch=400, 
                                ...
                                callbacks=[es, lr_cb, mc], initial_epoch=35)

with open('CNN_2_history.pickle', 'wb') as handle:
    pickle.dump(history.history, handle, protocol=pickle.HIGHEST_PROTOCOL)
network.save_model('CNN_2_model/')
...
try:
    network.model_train.save_weights('CNN_2_model/CNN_weights.h5')
except:
    pass
...
with open('CNN_2_model/CNN_accuracies.pickle', 'wb') as handle:
