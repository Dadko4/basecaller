...
class DataSet(object):
    ...
    def next_batch(self, batch_size, shuffle=True):
        ...
        # y_list = []
        # y_lens = []
        # for sam in label_batch[2]:
        #     sam[0].extend([0]*(300-sam[1]))
        #     y_list.append(sam[0])
        ...
        # #     label_batch = batch2sparse(label_batch)
        # seq_length = event_batch[:, 1].astype(np.int32)
        # events = [np.array(x).reshape(len(event_batch[:, 0][0]), 1) 
        #           for x in event_batch[:, 0]]
        # return np.array(events).astype(np.float32), seq_length, label_batch
